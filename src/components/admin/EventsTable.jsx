...
export const EventsTable = ({ events, onEdit, onDelete }) => {
  if (events.length === 0) {
    return (
      <tr>
  ...
  return (
    <>
      ...
      <tbody>
        {events.map(item => (
          <tr key={item.eid} className="hover:bg-gray-50">
            ...
            <td className="px-6 py-4 whitespace-nowrap text-sm space-x-2">
              ...
              <button onClick={() => onDelete(item.eid)} className="text-red-600 hover:text-red-900">Eliminar</button>
            </td>
          </tr>
        ))}
      </tbody>
